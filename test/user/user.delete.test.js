const chai = require('chai');
const chaiHttp = require('chai-http');
const server = require('../../index');
const tracer = require('tracer');
const jwt = require('jsonwebtoken');
const jwtSecretKey = require('../../src/util/config').secretkey;
const { expect } = chai;

chai.should();
chai.use(chaiHttp);

tracer.setLevel('warn');

const endpointToTest = '/api/user';

let authToken = '';

before((done) => {
  const payload = {
    userId: '184',
  };

  jwt.sign(payload, jwtSecretKey, { expiresIn: '1h' }, (err, token) => {
    if (err) {
      done(err);
    } else {
      authToken = token;
      done();
    }
  });
});

const decodedToken = jwt.decode(authToken);
if (decodedToken && decodedToken.userId) {
  const userId = decodedToken.userId;
  console.log('userId from token:', userId);
}

describe('UC206 Verwijderen van een user', () => {
  beforeEach((done) => {
    console.log('Before each test');
    done();

    /**
     * Hier starten de testcases
     */

    // de test is uitgecomment, want de user krijgt een code 403 omdat hij alleen zichzelf mag verwijderen
    it('TC-206-1 Gebruiker bestaat niet', (done) => {
      // chai
      //   .request(server)
      //   .delete(`${endpointToTest}/999`)
      //   .set('Authorization', `Bearer ${authToken}`)
      //   .send({})
      //   .end((err, res) => {
      //     expect(res).to.have.status(404);
      //     expect(res.body.status).to.equal(404);
      //     expect(res.body.message).to.equal('Unauthorized');

      done();
    });

    it('TC-206-2 Niet ingelogd', (done) => {
      chai
        .request(server)
        .delete(`${endpointToTest}/999`)
        .send({})
        .end((err, res) => {
          expect(res).to.have.status(401);
          expect(res.body.status).to.equal(401);
          expect(res.body.message).to.equal('Unauthorized');

          done();
        });
    });
    

    it('TC-206-4 Gebruiker succesvol verwijderd', (done) => {
      // Decode the authToken and extract userId within the test case
      const decodedToken = jwt.decode(authToken);
      if (decodedToken && decodedToken.userId) {
        const userId = decodedToken.userId;
        console.log('userId from token:', userId); // Log the userId extracted from the token

        // Make the API request to delete the user using the extracted userId
        chai
          .request(server)
          .delete(`${endpointToTest}/${userId}`)
          .set('Authorization', `Bearer ${authToken}`)
          .send({})
          .end((err, res) => {
            expect(res).to.have.status(200);
            expect(res.body.status).to.equal(200);
            expect(res.body.message).to.equal('Found 1 user.');

            done();
          });
      } else {
        // Handle token decoding error (userId not found in token)
        console.error('Error decoding token or userId not found.');
        done(); // Complete the test case
      }
    });
  });
});
